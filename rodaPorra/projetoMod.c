...
static char *key = "cachorrocachorrocachorrocachorro";
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t opcao, loff_t *offset){
	...
	for(i = 0; i<len; i++){     // c "oi"
             dadosBuffer[i] = kernelBuffer[i];
	}
	dadosBuffer[i] = '\0';
	
	for(i = 0; i < 16; i++)
		printk(KERN_INFO "HexOriginal: %02x\n", (unsigned char)buffer[i]);
	
	//printk(KERN_INFO "kernelBuff = %s", dadosBuffer);
	
	//printk(KERN_INFO "EBBChar: ANTES DO IF...\n");
	
     if(opcao == 'c') { // se for igual a c, o usuario escolheu a opcao Cifrar
           sprintf(message, "\n\nOP1\n\n");
	       criptar(dadosBuffer, strlen(dadosBuffer));
     } 
     if(opcao == 'd') { // se for igual a d, o usuario escolheu a opcao Decodificar
          sprintf(message, "\n\nOP2\n\n");
          decriptar(dadosBuffer, strlen(dadosBuffer));
     } 
     if(opcao == 'h') { // se for igual a h, o usuario escolheu a opcao Hash   
          sprintf(message, "\n\nOP3\n\n");
...
struct skcipher_def sk;
    ...
    for(i = 0; i < 16; i++)
		printk(KERN_INFO "dentroDECifrar: %02x\n", (unsigned char)buffer[i]);
...
static int criptar(char *buffer, size_t len){
   ...
   get_random_bytes(ivdata, 16);
	...
	// Decriptar 
	decriptar(retorno, strlen(retorno));
	
	/*
    strcpy(scratchpad, retorno);
    for(i = 0; i < 16; i++)
	printk(KERN_INFO "Scratchpad: %02x\n", (unsigned char)scratchpad[i]);
